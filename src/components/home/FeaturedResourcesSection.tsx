    ...
    <section className="mt-16 text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {resources.map((resource, index) => (
          <Card key={index} className="hover:shadow-md transition-all duration-300 border-emirati-desertGold/30 hover:border-emirati-desertGold group overflow-hidden">
            <div className="absolute inset-0 bg-gradient-to-b from-emirati-oasisGreen/5 to-transparent"></div>
            <CardHeader>
              <div className="flex items-center justify-center bg-emirati-oasisGreen/10 w-12 h-12 rounded-full mx-auto mb-2 group-hover:scale-110 transition-transform">
                {resource.icon}
              </div>
